"""
modal_train.py — H100 Training Script for MIA Diffusion (v2 AMP)
=================================================================
Correct LiRA experimental design:
  - Universe = ALL 50K CIFAR-10 samples
  - Target trains on random 25K (50% of universe)
  - Each of 16 shadow models also trains on random 25K of the SAME 50K universe
  - This gives proper N_IN and N_OUT shadow distributions for EVERY attack sample

Speed optimisations applied (train.py + this file):
  - AMP (torch.cuda.amp) in train loop  -- 2-2.5x throughput on H100
  - autocast in eval_losses              -- faster shadow evaluation pass
  - batch_size=128, lr=2e-4              -- better GPU utilisation
  - AdamW + cosine LR                    -- cleaner convergence

Estimated cost @ H100 $0.001097/sec with AMP:
  target  800 epochs  ~ 27 min  ~ $1.8
  shadow  400 epochs  ~ 13 min each x16  ~ $14.0
  TOTAL   ~ $16  (leaves ~$14 of your $30 budget as safety margin)

WHAT GETS SAVED TO VOLUME 'mia-diffusion-results' -> /outputs/:
  config.json               -- full CFG dict
  target_idx.json           -- 25K indices the target model trained on
  target_sample_idx.json    -- n_eval member indices for Kaggle eval
  nonmember_idx.json        -- n_nonmember indices for Kaggle eval
  target_model.pth          -- target model weights
  shadow_model_1.pth...16   -- shadow model weights
  shadow_losses.npy         -- float32 (16, len(t_sweep), 2*n_probe)
                               layout: [:,:,:n_probe]=members | rest=non-members
  shadow_membership.npy     -- bool (16, 2*n_probe) True if shadow trained on sample

AFTER THIS JOB:
  1. python download_from_modal.py --out_dir ./mia_downloads
  2. Upload mia_downloads/ folder as Private Kaggle Dataset
  3. Open eval_kaggle.ipynb in Kaggle -> Add Dataset -> Run All
"""
